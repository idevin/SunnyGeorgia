          ...
          <div class="local-weather">
            <div class="local-weather__main">
              <div class="local-weather__main-item row">
                <div class="col-8">
                  <div>
                    <small>{{ weatherFirst | dateFormatter }}</small>
                  </div>
                  <div>
                    <span>{{ weatherFirst.summary }}</span>
                  </div>
                </div>
                <div class="col-4" style="text-align: center">
                  <div class="icon-w sun-shower" v-if="false">
                    <div class="cloud"></div>
                    <div class="sun">
                      <div class="rays"></div>
                    </div>
                  ...
                  <div class="icon-w thunder-storm" v-if="weatherFirst.icon == 'thunderstorm' || weatherFirst.icon == 'tornado'">
                    <div class="cloud"></div>
                    <div class="lightning">
                      <div class="bolt"></div>
                      <div class="bolt"></div>
                  ...
                  <div
                    class="icon-w cloudy"
                    v-if="
                      weatherFirst.icon == 'partly-cloudy-day' ||
                        weatherFirst.icon == 'partly-cloudy-night' ||
                        weatherFirst.icon == 'wind' ||
                        weatherFirst.icon == 'fog' ||
                        weatherFirst.icon == 'cloudy'
                    "
                  >
                    <div class="cloud"></div>
                    <div class="cloud"></div>
                  </div>
                  <div class="icon-w flurries" v-if="weatherFirst.icon == 'snow' || weatherFirst.icon == 'sleet'">
                    <div class="cloud"></div>
                    <div class="snow">
                      <div class="flake"></div>
                      <div class="flake"></div>
                  ...
                  <div class="icon-w rainy" v-if="weatherFirst.icon == 'rain'">
                    <div class="cloud"></div>
                    <div class="rain"></div>
                  </div>
                  <div class="icon-w sunny" v-if="weatherFirst.icon == 'clear-day' || weatherFirst.icon == 'clear-night'">
                    <div class="sun">
                      <div class="rays"></div>
                    </div>
                  </div>
              ...
              <div class="local-weather__header">
                <div class="row">
                  <div class="local-weather__title col-6">
                ...
                <div class="weather-forecast">
                  <div class="weather-forecast__item" v-for="(item, index) in weather">
                    ...
                    <div class="weather-forecast__content">
                      <span
                      ><small>{{ item | temperatureSymbol }}</small> {{ item | averageTemperature }}</span
                      >
                      <!--                                    <div>-->
                      <!--                                        <icon v-if="item.icon == 'clear-day' || item.icon == 'clear-night'" name="sun" style="color: #e8b912;" scale="1"></icon>-->
...
<script>
const locale = window.document.documentElement.lang;
...
export default {
  ...
  filters: {
    temperatureSymbol: function(item) {
      const temp = parseInt((item.apparentTemperatureMax + item.apparentTemperatureMin) / 2);
      if (temp > 0) {
        return '+';
      } else if (temp < 0) {
        return '-';
    ...
    dateFormatter: function(item) {
      return new Date(item.time * 1000).toLocaleDateString(locale, {
        month: 'long',
        day: 'numeric',
      });
    },
  ...
  methods: {
    loadWeather() {
      ...
      if (self.place.lat && self.place.long) {
        axios
          .get('/xhr/weather?lat=' + self.place.lat + '&long=' + self.place.long + '&lang=' + window.document.documentElement.lang)
...
.cloud {
  ...
  background: #e5e5e5;
  border-radius: 50%;
  box-shadow: -2.1875em 0.6875em 0 -0.6875em #e5e5e5, 2.0625em 0.9375em 0 -0.9375em #e5e5e5, 0 0 0 0.375em #fff,
