<template>
    <div class="int-tel-input-wapper clear">
        <div id="select" @click="selectInputClickHandle" class="int-tel-select" ref="select" :class="{'invalid' : state == 'invalid'}">
            <span :class="selectedInputClass" id="selectFlag" class="int-countries-flag"></span>
            <div :class="{'is-reverse': visible}" class="int-tel-icon"></div>
        </div>
        <transition name="zoom-in-top">
            <div v-show="visible" id="selectDropdown" ref="selectDropdown" class="select-dropdown">
                <div class="scrollbar__wrap">
                    <ul id="scrollbar" class="scrollbar">
                        <li v-for="(item, key) in calculatePhoneData" :class="{'selected': countryAbbr === item.addr}" @click.stop="selectOptionHandle(item)" :key="key" class="options clear">
                            <span :class="item.addr" class="int-countries-flag"></span>
                            <span v-text="item.name" class="int-countries-name"></span>
                            <span v-text="`+${item.code}`" class="int-countries-addr"></span>
                        </li>
                    </ul>
                </div>
            </div>
        </transition>
        <div class="int-tel-input">
            <input type="text" :value="phone" @input="phoneInputHandel" :placeholder="placeholder" class="form-control" id="input" :class="{'is-invalid' : state == 'invalid'}">
            <div v-if="state === 'invalid'" class="invalid-feedback" id="inputFeedback">{{ stateFeedback }}</div>
        </div>
    </div>
</template>
...
<script>
    ...
    export default {
        props: [
            "value", "placeholder", "state", "stateFeedback"
        ],
        data() {
            return {
                phone:null,
        ...
        methods: {
            ...
            selectOptionHandle(value) {
                this.visible     = false;
                this.phone       = '+' + value.code;
                this.countryAbbr = value.addr;
            ...
            setCountryCode(phone) {
                ...
                let countryCode = Object.keys(phonesData).forEach((code) => {
                    if (phone.indexOf(phonesData[code].code, prefix) === prefix) {
                        this.countryAbbr = String(phonesData[code].addr);
                    }
                });
            },
        ...
        mounted() {
            if (this.phone) {
                this.setCountryCode(this.phone);
            }
...
<style>
    ...
    .int-tel-input-wapper .int-tel-select .int-tel-icon {
        position: absolute;
        top: 0;
        right: 5px;
        bottom: 0;
    ...
    .int-tel-input-wapper .int-tel-select .int-tel-icon.is-reverse {
    ...
    .int-tel-input-wapper .int-tel-select>input:hover,
    .int-tel-input-wapper .int-tel-select>input:focus {
        border-color: #eee;
        z-index: 1;
    }

    .int-tel-input-wapper .select-dropdown {
        position: absolute;
        z-index: 2;
        top: 100%;
        left: 0;
    ...
    .int-tel-input-wapper .select-dropdown .scrollbar__wrap {
        border: solid 1px #e0e0e0;
    }

    .int-tel-input-wapper .select-dropdown .scrollbar__wrap .filter-wapper .filter-input {
        position: relative;
        padding-left: 10px;
        padding-right: 35px;
        margin-left: -1px;
    ...
    .int-tel-input-wapper .select-dropdown .scrollbar__wrap .filter-wapper .filter-input::after {
        position: absolute;
        right: 0;
        top: 0;
        width: 34px;
    ...
    .int-tel-input-wapper .select-dropdown .scrollbar__wrap .filter-wapper .filter-input input {
        width: 100%;
        height: 34px;
        line-height: 34px;
    }

    .int-tel-input-wapper .select-dropdown .scrollbar__wrap .filter-wapper .filter-input input::placeholder {
        font-size: 12px;
    }

    .int-tel-input-wapper .select-dropdown .scrollbar__wrap .filter-wapper .filter-alphabet {
        padding: 5px;
        border-bottom: 1px solid #f1f1f1;
    }

    .int-tel-input-wapper .select-dropdown .scrollbar__wrap .filter-wapper .filter-alphabet p {
        float: left;
        width: 18px;
        height: 18px;
        line-height: 18px;
    ...
    .int-tel-input-wapper .select-dropdown .scrollbar__wrap .filter-wapper .filter-alphabet .selected {
        color: #857dd1;
        background: #ececec;
        border-radius: 2px;
    }

    .int-tel-input-wapper .select-dropdown .scrollbar__wrap .scrollbar {
        max-height: 200px;
        overflow-y: auto;
        margin: 0;
        padding-top: 5px;
    ...
    .int-tel-input-wapper .select-dropdown .scrollbar__wrap .scrollbar li.options.no-result {
        text-align: center;
    }
    ...
    .int-tel-input-wapper .int-tel-select .int-countries-flag {
        display: inline-block;
        float: left;
        width: 20px;
        height: 15px;
    ...
    .int-tel-input-wapper .select-dropdown .scrollbar__wrap .scrollbar li.options .int-countries-name {
        max-width: 60%;
        overflow: hidden;
        text-overflow: ellipsis;
        float: left;
    ...
    .int-tel-input-wapper .select-dropdown .scrollbar__wrap .scrollbar li.options .int-countries-addr {
        float: right;
    }
    ...
    .int-tel-input-wapper .int-tel-input input:hover,
    .int-tel-input-wapper .int-tel-input input:focus {
        box-shadow: -1px 0px #ececec;
        border-color: #eee;
        z-index: 1;
    }
    ...
    .int-tel-input .invalid-feedback {
        margin-left: -60px;
    }

    .int-tel-select.invalid {
