                ...
                <div class="m-portlet__head-tools">
                    <div class="btn-group btn-group-lg" role="group" aria-label="Button group with nested dropdown">
                        ...
                        <a href="{{route('cabinet:partner:tours.accommodation', $tour->id)}}"
                           class="btn btn-default active"><i class="fa fa-building"></i></a>
        ...
        <div class="m-content">
            <div class="m-portlet m-portlet--mobile">
                ...
                <div class="m-portlet__body">
                    <form action="{{route('cabinet:partner:tours.accommodation.update', $tour->id)}}" method="POST">
                        {!! csrf_field() !!}
                                ...
                                <tr>
                                    ...
                                    @foreach($rooms as $m => $room)
                                        <td @if($m % 2 == 0) style="background-color: #d9edf7;" @endif>
                                            <input name="acoms[{{$n}}][{{$m}}][price_adult]" type="number" step="0.01" min="0" class="form-control"
                        ...
                        <a href="{{route('cabinet:partner:tours.accommodation', $tour->id)}}" class="btn btn-default">{{trans('main.Cancel')}}</a>
...
{{--OLD TOOR PRICES--}}
    {{--<div class="m-portlet__body">
        <table class="table">
            ...
            @foreach($acom as $item)
                <tr>
                    ...
                    <td>{{$item->room}}</td>
                    <td>{{$item->adults}}</td>
                    <td>{{$item->kids or 0}}</td>
                    <td>{{$item->add_beds or 0}}</td>
                    <td align="right">
                    ...
                    --}}{{--</td>--}}{{--
                    <td>
                        <a href="{{route('cabinet:partner:tours.accommodation.delete', [$tour->id, $item->id])}}"><i
        ...
        {{--            <div class="m-portlet m-portlet--mobile">
                        ...
                        <div class="m-portlet__body">
                            <form action="{{route('cabinet:partner:tours.accommodation.post', $tour->id)}}" method="post">
                                {!! csrf_field() !!}
                                <table class="table">
                                    <tr>
                                        <th>Отель</th>
