    ...
    ">
            ...
            <div class="m-portlet__head-caption">
                <div class="m-portlet__head-title">
                    ...
                    <h3 class="m-portlet__head-text">
                        Бронирование экскурсии #{{$order->id}}<small>{{$order->created_at}}</small>
                    </h3>
                </div>
            </div>
        ...
        <div class="m-portlet__body">
            <div class="row">
                <div class="col-lg-8">
                    <h4>
                        <a href="{{route('excursions.view', ['excursion' => $order->excursion->slug])}}"
                           target="_blank"
                        >{{str_limit($order->excursion->title, 200)}}</a>
                    </h4>
                    ...
                    <div class="m--margin-15">
                        @if($order->status == 'confirmed')
                            <h5>{{trans('cabinet/index.Status')}}:
                                <span class="m-badge m-badge--primary m-badge--wide m--font-boldest">Подтвержден</span>
                            </h5>
                        @elseif($order->status == 'created')
                            <h5>{{trans('cabinet/index.Status')}}:
                                <span class="badge badge-warning">Ожидаем подтверждение</span>
                            </h5>
                        @elseif($order->status == 'payed')
                            <h5>{{trans('cabinet/index.Status')}}:
                        ...
                        @elseif($order->status == 'canceled')
                            <h5>{{trans('cabinet/index.Status')}}:
                                <span class="badge badge-info">{{trans('cabinet/index.Canceled')}}</span>
                            </h5>
                        @endif
                    </div>
                </div>
        ...
        <div class="m-portlet__foot">
            <a href="{{route('cabinet:orders.excursion_view', $order->id)}}" class="btn btn-primary">Подробно</a>
            @if($order->status == 'confirmed')
                <a class="btn m-btn btn-success"
                   href="{{route('cabinet:orders.excursion_view', $order->id)}}"
